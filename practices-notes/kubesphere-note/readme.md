...
安装完成的话，你会在最后看到一个类似是欢迎界面一样的信息。上面有管理员账户的初始密码以及一个可用的访问地址。

这是一个完成安装后，执行上面命令得到的标准输出的最后几十行的内容：
...
你看到的可能会是它的输出中途，那就得再等等。

……所以， Kubesphare 真的很简单，而且很直观。似乎也根本用不到他们的 `kubekey` 工具……
