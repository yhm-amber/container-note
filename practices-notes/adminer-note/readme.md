...
它本身是无状态的（是个 `php` 写的数据库客户端），并且是个 Web 应用（虽然有界面但是要用浏览器访问嘛）。

这使得，如果想要做一些简单的示例，它也是理想的选择。下面就分别是涉及到了 `kubectl` 创建资源以及涉及到了 `helm chart` 模板编写的两个例子。
